...
12. 完整翻译完成后，如有值得解释的关键词或短语，应出现独立“关键词汇”区域；如果没有值得解释的内容，不应显示该区域。
13. 点击“复制译文”，剪贴板内容应只包含完整译文，不包含“关键词汇”内容。
14. 点击“解释用法 / 给例句 / 语法分析”，应默认针对原文追问；点击“更自然表达”，应基于原文和参考译文输出更自然的目标语言译法。
15. 在底部输入自由追问并发送，应默认针对原文追问；明确询问译文时才分析译文。
16. 关闭面板后再次翻译，上一轮追问上下文不应保留。
17. 使用错误 API Key，应在面板内提示 API Key 或权限错误。
18. 使用错误 model，应在面板内提示模型名或 `base_url` 可能错误。
